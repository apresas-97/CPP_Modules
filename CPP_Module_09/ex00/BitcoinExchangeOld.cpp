...
void	BitcoinExchange::checkConversion( std::string & filename )
{
	...
	std::vector<std::string> input = getVectorFromFile2(file);
	std::vector<std::string>::iterator it = input.begin();
	std::vector<std::string>::iterator it_data = this->_data.begin();
	while (it != input.end())
	{
...
std::vector<std::string>	BitcoinExchange::getVectorFromFile2( std::ifstream & file )
{
