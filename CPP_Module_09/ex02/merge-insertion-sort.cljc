...
(defn sort
  ...
  ([comp coll]
   (if (< (count coll) 2)  ; If coll is empty or has only one element, it is already sorted
     coll ; return coll as is
     (let [comp (fn->comparator comp) ; convert comp to a comparator function
           sorted-pairs (->> coll ; sort coll into pairs [[a b] [a b] ...]
